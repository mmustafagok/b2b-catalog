...
export function selectDatabaseUrlSource(env = process.env) {
  return env.WORKER_DATABASE_URL ? 'WORKER_DATABASE_URL' : 'DATABASE_URL';
}

export function runMigration(env = process.env) {
  const source = selectDatabaseUrlSource(env);
  ...
  if (!normalized) {
    console.error(`[migrate-production] FATAL: ${source} is not set. Exiting.`);
    process.exit(1);
  }
  ...
  const childEnv = { ...env, DATABASE_URL: normalized };
...
const isMain = process.argv[1] && (
  process.argv[1].endsWith('migrate-production.mjs') ||
  process.argv[1].endsWith('migrate-production.js')
);

if (isMain) {
  process.exit(runMigration());
}
